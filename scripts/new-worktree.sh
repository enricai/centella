...
#
# The worktree branches off the CURRENT centella/runs/<run-id> tip, so a
...
#
# Branch shape: subtask branches live under `centella/subtasks/<run-id>/<sid>`,
# disjoint from the run-branch namespace `centella/runs/<run-id>`. The two
...
BRANCH="centella/subtasks/${RUN_ID}/${ID}"
PARENT_BRANCH="centella/runs/${RUN_ID}"
