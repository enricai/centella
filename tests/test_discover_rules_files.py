...
def test_returns_empty_when_no_rules_files_exist(centella, tmp_path):
    assert centella.discover_rules_files(tmp_path) == []


def test_finds_claude_md_at_root(centella, tmp_path):
    (tmp_path / "CLAUDE.md").write_text("# rules\n")
    out = centella.discover_rules_files(tmp_path)
    assert out == [tmp_path / "CLAUDE.md"]


def test_finds_agents_md_at_root(centella, tmp_path):
    (tmp_path / "AGENTS.md").write_text("# rules\n")
    out = centella.discover_rules_files(tmp_path)
    assert out == [tmp_path / "AGENTS.md"]


def test_finds_readme_when_present(centella, tmp_path):
    (tmp_path / "README.md").write_text("# readme\n")
    out = centella.discover_rules_files(tmp_path)
    assert out == [tmp_path / "README.md"]


def test_finds_docs_files(centella, tmp_path):
    docs = tmp_path / "docs"
    docs.mkdir()
...
def test_returns_priority_order_not_filesystem_order(centella, tmp_path):
...
def test_capped_at_allowlist_length(centella, tmp_path):
    ...
    for rel in centella._RULES_FILE_CANDIDATES:
        p = tmp_path / rel
    ...
    out = centella.discover_rules_files(tmp_path)
    assert len(out) == len(centella._RULES_FILE_CANDIDATES)
    rels = [p.relative_to(tmp_path).as_posix() for p in out]
    assert "RANDOM_RULES.md" not in rels


def test_directory_with_candidate_name_is_skipped(centella, tmp_path):
...
def test_nonexistent_repo_root_returns_empty(centella, tmp_path):
    """A repo root that doesn't exist returns [] without raising — the
    contract is "never raises."""
    out = centella.discover_rules_files(tmp_path / "does-not-exist")
    assert out == []
