...
def test_no_push_short_circuits(centella, monkeypatch):
    ...
    centella._check_gh_cli(no_push=True)
    assert called == [], (
...
def test_gh_not_on_path_dies(centella, monkeypatch):
    monkeypatch.setattr(centella.shutil, "which", lambda _: None)
    with pytest.raises(SystemExit):
        centella._check_gh_cli(no_push=False)


def test_gh_auth_failure_dies(centella, monkeypatch):
    """gh exists but `gh auth status` exits non-zero → die with the
    login hint."""
    monkeypatch.setattr(centella.shutil, "which", lambda _: "/usr/bin/gh")

    def run(cmd, **kwargs):
        if cmd[:3] == ["gh", "auth", "status"]:
            return _fake_run(1, stderr="not logged in")
        return _fake_run(0)
    monkeypatch.setattr(centella.subprocess, "run", run)
    with pytest.raises(SystemExit):
        centella._check_gh_cli(no_push=False)


def test_no_origin_remote_dies(centella, monkeypatch):
...
def test_all_checks_pass_returns_silently(centella, monkeypatch):
    ...
    monkeypatch.setattr(centella.subprocess, "run", run)
    # Must not raise.
    assert centella._check_gh_cli(no_push=False) is None


def test_error_messages_mention_no_push_escape_hatch(centella, monkeypatch, capsys):
