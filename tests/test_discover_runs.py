"""Tests for `discover_runs()` — enumerates `.centella/runs/*/state.json`
...
def _make_run(centella_root: Path, run_id: str, state: dict) -> Path:
...
def test_discover_runs_empty_dir(centella, tmp_path):
    """No `.centella/runs/` directory → empty list, no error."""
    assert centella.discover_runs(tmp_path) == []


def test_discover_runs_empty_runs_dir(centella, tmp_path):
    """`.centella/runs/` exists but has no children → empty list."""
    (tmp_path / "runs").mkdir()
    assert centella.discover_runs(tmp_path) == []


def test_discover_runs_single_run(centella, tmp_path):
    _make_run(tmp_path, "feat-foo-abc123",
              {"task": "do thing", "started_at": "2026-05-26T10:00:00+00:00"})
    runs = centella.discover_runs(tmp_path)
    assert len(runs) == 1
...
def test_discover_runs_multiple_runs_sorted_newest_first(centella, tmp_path):
...
def test_discover_runs_skips_bootstrap_dirs(centella, tmp_path):
...
def test_discover_runs_skips_non_dirs(centella, tmp_path):
...
def test_discover_runs_skips_dirs_without_state_json(centella, tmp_path):
...
def test_discover_runs_skips_malformed_json(centella, tmp_path, capsys):
...
def test_discover_runs_skips_non_object_state(centella, tmp_path):
    """state.json that contains valid JSON but not an object (array,
    string) is still useless to centella; skip it."""
...
def test_discover_runs_handles_missing_started_at(centella, tmp_path):
...
def test_discover_runs_preserves_state_fields(centella, tmp_path):
