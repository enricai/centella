...
historical centella runs: 0 false positives in the n=3 runs available
...
def _capture_logs(centella, monkeypatch):
    """Capture every `log()` call so the test can assert on emitted lines."""
    lines: list[str] = []
    monkeypatch.setattr(centella, "log", lambda msg: lines.append(msg))
    return lines


def test_no_overlap_is_silent(centella, monkeypatch):
    lines = _capture_logs(centella, monkeypatch)
    plans = [
    ...
    centella.warn_cross_planner_file_overlap(plans)
    assert lines == []


def test_overlap_within_same_planner_does_not_warn(centella, monkeypatch):
...
def test_cross_planner_overlap_warns(centella, monkeypatch):
...
def test_warn_reports_count(centella, monkeypatch):
    lines = _capture_logs(centella, monkeypatch)
    plans = [
        {"domain": "feat", "subtasks": [
            {"id": "feat-001",
...
def test_multiple_subtasks_per_planner_listed(centella, monkeypatch):
...
def test_empty_plans_list(centella, monkeypatch):
    lines = _capture_logs(centella, monkeypatch)
    centella.warn_cross_planner_file_overlap([])
    assert lines == []


def test_plan_with_no_subtasks(centella, monkeypatch):
...
def test_missing_files_likely_touched_is_safe(centella, monkeypatch):
