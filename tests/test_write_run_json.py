...
def test_overwrites_existing_field(centella, tmp_path):
    """A field provided in a later call overrides the earlier value."""
    centella._write_run_json(tmp_path, push_error="first failure")
    centella._write_run_json(tmp_path, push_error=None,
...
def test_writes_null_for_none_values(centella, tmp_path):
    """None-valued fields are written as JSON null, not omitted.
    Used to clear a prior error explicitly."""
    centella._write_run_json(tmp_path, push_error=None, pr_url=None)
...
def test_atomic_write_via_temp_rename(centella, tmp_path):
...
def test_rejects_invariant_violation(centella, tmp_path):
    ...
    with pytest.raises(ValueError, match="PR cannot succeed"):
        centella._write_run_json(tmp_path, pr_url="https://gh.com/pr/1")
    # And nothing got written.
    assert not (tmp_path / "run.json").exists()


def test_rejects_violation_on_update(centella, tmp_path):
    ...
    with pytest.raises(ValueError, match="pushed_at and push_error"):
        centella._write_run_json(tmp_path, push_error="conflict somehow")
    # Existing sidecar is intact.
    data = json.loads((tmp_path / "run.json").read_text())
    assert data["pushed_at"] == "2026-05-26T15:00:00+00:00"


def test_malformed_existing_is_recovered(centella, tmp_path):
    ...
    centella._write_run_json(tmp_path, run_id="recovered")
    data = json.loads((tmp_path / "run.json").read_text())
    assert data == {"run_id": "recovered"}


def test_non_object_existing_is_recovered(centella, tmp_path):
...
def test_full_lifecycle(centella, tmp_path):
