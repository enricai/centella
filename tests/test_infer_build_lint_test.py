...
def _infer(centella, tmp_path):
    return centella._infer_build_lint_test(tmp_path)


def test_empty_repo_returns_all_empty(centella, tmp_path):
    blt = _infer(centella, tmp_path)
    assert blt == {"build": "", "lint": "", "test": ""}


def test_pyproject_only_infers_pytest(centella, tmp_path):
...
def test_package_json_infers_npm_build_and_test(centella, tmp_path):
...
def test_cargo_infers_cargo_commands(centella, tmp_path):
...
def test_go_mod_infers_go_commands(centella, tmp_path):
...
def test_eslintrc_classic_infers_eslint(centella, tmp_path):
    (tmp_path / ".eslintrc").write_text("{}")
    blt = _infer(centella, tmp_path)
    assert blt["lint"] == "npx eslint ."


def test_eslintrc_json_infers_eslint(centella, tmp_path):
    (tmp_path / ".eslintrc.json").write_text("{}")
    assert _infer(centella, tmp_path)["lint"] == "npx eslint ."


def test_eslintrc_js_infers_eslint(centella, tmp_path):
    (tmp_path / ".eslintrc.js").write_text("module.exports = {};")
    assert _infer(centella, tmp_path)["lint"] == "npx eslint ."


def test_eslintrc_cjs_infers_eslint(centella, tmp_path):
...
def test_eslintrc_yaml_infers_eslint(centella, tmp_path):
    """Third-pass audit follow-up — .eslintrc.yaml variant."""
    (tmp_path / ".eslintrc.yaml").write_text("env:\n  node: true\n")
    assert _infer(centella, tmp_path)["lint"] == "npx eslint ."


def test_eslintrc_yml_infers_eslint(centella, tmp_path):
    """Third-pass audit follow-up — .eslintrc.yml variant."""
    (tmp_path / ".eslintrc.yml").write_text("env:\n  node: true\n")
    assert _infer(centella, tmp_path)["lint"] == "npx eslint ."


def test_ruff_toml_infers_ruff(centella, tmp_path):
    (tmp_path / "ruff.toml").write_text("line-length = 100\n")
    assert _infer(centella, tmp_path)["lint"] == "ruff check ."


def test_polyglot_node_python_picks_npm_build(centella, tmp_path):
...
def test_makefile_infers_make(centella, tmp_path):
    (tmp_path / "Makefile").write_text("all:\n\techo ok\n")
    assert _infer(centella, tmp_path)["build"] == "make"
