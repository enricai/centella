...
The original branch scheme used `centella/<run-id>` for the run branch
and `centella/<run-id>/<sid>` for each subtask branch. This crashed
every run at the first `git worktree add` for a subtask:

  fatal: cannot lock ref 'refs/heads/centella/<run-id>/feat-001':
    'refs/heads/centella/<run-id>' exists;
    cannot create 'refs/heads/centella/<run-id>/feat-001'
...
under disjoint sub-prefixes (`centella/runs/...` vs.
`centella/subtasks/...`) so neither is an ancestor of the other.
...
def test_is_ref_ancestor_helper_sanity():
    ...
    assert _is_ref_ancestor("centella/myrun", "centella/myrun/feat-001") is True
    # Sibling refs — fine.
    assert _is_ref_ancestor("centella/runs/myrun", "centella/subtasks/myrun/feat-001") is False
    # Identical refs — not "ancestor" in the strict sense.
    assert _is_ref_ancestor("centella/runs/myrun", "centella/runs/myrun") is False
    # Reverse direction.
    assert _is_ref_ancestor("centella/runs/myrun/feat-001", "centella/runs/myrun") is False


@pytest.mark.parametrize("run_id,sid", _SAMPLES)
def test_run_branch_is_not_subtask_branch_ancestor(centella, run_id, sid):
    ...
    run_branch = centella.compute_run_branch(run_id)
    subtask_branch = centella.compute_subtask_branch(run_id, sid)
...
@pytest.mark.parametrize("run_id,sid", _SAMPLES)
def test_subtask_branches_for_same_run_dont_collide(centella, run_id, sid):
    ...
    would fail."""
    branch_a = centella.compute_subtask_branch(run_id, sid)
    branch_b = centella.compute_subtask_branch(run_id, sid + "-other")
    assert not _is_ref_ancestor(branch_a, branch_b)
    assert not _is_ref_ancestor(branch_b, branch_a)


def test_namespaces_are_explicitly_disjoint(centella):
    ...
    that uses, say, `centella/run/<id>` and `centella/run/<id>/<sid>`
    fails fast at this test rather than slipping past the ancestor
    check on some adversarial input."""
    run_branch = centella.compute_run_branch("any-id-aaaaaa")
    subtask_branch = centella.compute_subtask_branch("any-id-aaaaaa", "feat-001")
    assert run_branch.split("/")[:2] == ["centella", "runs"]
    assert subtask_branch.split("/")[:2] == ["centella", "subtasks"]
