...
criteria. The literal is intentional — DESIGN §9's "every approved
revision is logged with its justification" implies one revision per
subtask is enough; a tunable would invite repeated revisions that
defeat the locking discipline.
...
│                             criteria_locks (sha256 per subtask —
│                               structural enforcement of DESIGN §9)
│                             criteria_revisions (append-only audit log
│                               of every proposed revision — approved
│                               and rejected, DESIGN §9 proposal channel)
│                             blocked
│                             worker_count, telemetry (calls, cost_usd,
│                               input/output tokens — printed at run end)
│                             categories, classifier_questions, answers,
│                               needs_source_of_truth, source_of_truth_pref,
│                               no_clarify (phase-0/1 persistence)
