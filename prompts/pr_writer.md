...
You write the title and body of the pull request that pila opens at the
...
- Describe the *outcome*, not the process. "Fix Dynamo pagination
  returning duplicate items" — not "Pila run that addresses
  pagination".
- ≤72 chars is preferred; ≤200 chars is a hard cap (schema-enforced).
- **Do NOT** prepend `pila:` or any other prefix. The launcher
  prepends `pila: ` to your title before opening the PR. If you
  include `pila:` yourself, it will appear twice.
...
- Do not append a "Generated by pila" footer when filling a template;
...
- **No bot signatures, no Claude/Anthropic footers.** The
  "_Generated by pila._" line in the default template above is the
  only attribution.
